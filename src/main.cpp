...
Net ageNet, genderNet;
std::vector<std::string> ageRanges = {"0-2", "4-6", "8-13", "15-20", "25-32", "38-43", "48-53", "60+"};
std::vector<std::string> genderList = {"M", "F"};
//----------------------------------------------------------------------------------------
using namespace std;
using namespace cv;
//----------------------------------------------------------------------------------------
...
//----- labels ----------------------------
    ...
    // Use full label with age/gender
    if (!obj.regName.empty()) {
        Str = obj.regName;
    } else {
...
// Age/Gender prediction function
void predictAgeGender(const cv::Mat& faceROI, std::string& gender, std::string& age) {
    // Preprocess: blob to 227x227 with mean subtract
    cv::Mat blob = blobFromImage(faceROI, 1.0, cv::Size(227, 227),
                                 cv::Scalar(78.4263377603, 87.7689143744, 114.895847746),
                                 false, false);

    // Gender: argmax on [1,2] output (0=M, 1=F)
    genderNet.setInput(blob);
    cv::Mat genderProb = genderNet.forward();
    cv::Point genderLoc;
    minMaxLoc(genderProb, nullptr, nullptr, nullptr, &genderLoc);
    gender = genderList[genderLoc.x];

    // Age: argmax on [1,8] output
    ageNet.setInput(blob);
    cv::Mat ageProb = ageNet.forward();
    cv::Point ageLoc;
    minMaxLoc(ageProb, nullptr, nullptr, nullptr, &ageLoc);
    age = ageRanges[ageLoc.x];
}
...
int main(int argc, char **argv)
{
    ...
    Live.LoadModel();
    // Load age/gender nets (with debug)
    // Load age/gender nets (paths relative to build/)
    ageNet = readNetFromCaffe("../models/age_gender/age_deploy.prototxt", "../models/age_gender/age_net.caffemodel");
    genderNet = readNetFromCaffe("../models/age_gender/gender_deploy.prototxt", "../models/age_gender/gender_net.caffemodel");
        ...
        if(Faces.size()==1){
            ...
            for(i=0;i<Faces.size();i++){
                if(Faces[i].FaceProb>MinFaceThreshold){
                    ...
                    // Predict age/gender on aligned face
                    std::string gender, ageGroup;
                    predictAgeGender(aligned, gender, ageGroup);
...
//                                cv::imshow("Add",aligned);
                    ...
                    // Build full label with age/gender (for recognized, stranger, or valid cases)
                    std::string fullLabel;
                    switch(Faces[i].NameIndex){
                        case -1: fullLabel = "Stranger"; break;
                        case -2: fullLabel = "too tiny"; break;
                        case -3: fullLabel = "Fake !"; break;
                        default: fullLabel = NameFaces[Faces[i].NameIndex];
                    }
                    if (Faces[i].NameIndex >= -1) {  // Only append if valid face (not tiny/fake)
                        fullLabel += " | " + gender + " | " + ageGroup;
                    }
                    Faces[i].regName = fullLabel;  // Use existing regName field if available, or add std::string regName; to FaceObject in TMtCNN.h (likely defined there—search for "struct FaceObject")
                }
            }
        }
#endif // RECOGNIZE_FACE
